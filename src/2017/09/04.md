...
このコンテナから detach することはないので、 detach-keys には押されないキーを指定してある。
...
- 実験はしていないが、このコマンドを実行すると、ユーザーでも docker コマンドが実行できるようになってしまうように感じる。 mac を共有して使用している場合はその辺り確かめる必要がある
...
VS Code 等のツールを使用して開発する場合はこれが必要なくなるだろう。
